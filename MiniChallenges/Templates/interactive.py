...
def challenge2c(): # selecting columns
    print(cbot(" Now that we have all the column names, let's choose a couple to look at. How about the "), code("'Name' "), normal("column?\n"), task("Select the column using ", check = True), code("dataset_variable_name[\"column_name\"]"))
...
def challenge2d(): # finding the longest name
    print(cbot(" Take a look at the table we generated with the "), code("'Name' "), normal("in the data.\n"), task("Type the longest name in the list ", check = True))
    def Q2D(): # select name column
        ans = str(input(normal("The longest name is: ")))
        longest = df['Name'].sort_values(key=lambda x: x.str.len(), ascending=False).iloc[0]
